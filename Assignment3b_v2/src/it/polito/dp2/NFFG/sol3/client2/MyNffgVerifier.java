...
public class MyNffgVerifier implements NffgVerifier {
	...
	public MyNffgVerifier(String url_address) throws NffgVerifierException {
		...
		Response getpolicy = target.path("policiesService/").request(MediaType.APPLICATION_XML).get();
		if (getpolicy.getStatus() != 200)
			throw new NffgVerifierException();
		List<Policy> policies = getNffg.readEntity(new GenericType<List<Policy>>() {
		});
		...
		for (Policy policyItem : policies) {
			NffgReader nffg_r = nffg_r_set.get(policyItem.getReachabilityPolicy().getNffgName());
			Nffg nffgItem = nffgMap.get(policyItem.getReachabilityPolicy().getNffgName());
