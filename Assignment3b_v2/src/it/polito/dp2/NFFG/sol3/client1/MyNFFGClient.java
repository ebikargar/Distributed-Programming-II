...
public class MyNFFGClient implements NFFGClient {
	...
	private void loadPolicyReader(PolicyReader policy_r) throws ServiceException {
		...
		Response delRes = target.path("policiesService").request(MediaType.APPLICATION_XML)
	...
	public void unloadReachabilityPolicy(String name) throws UnknownNameException, ServiceException {
		Response delRes = target.path("policiesService/" + name).request(MediaType.APPLICATION_XML).delete();
		if (delRes.getStatus() == 404) {
			System.out.println("Not found policy to be delete ...");
			throw new UnknownNameException();
		}
	...
	public boolean testReachabilityPolicy(String name) throws UnknownNameException, ServiceException {
		...
		Response delRes = target.path("policiesService/" + name + "/verificationResult")
				.request(MediaType.APPLICATION_XML).post(Entity.entity(null, MediaType.APPLICATION_XML));
		if (delRes.getStatus() == 404) {
		...
		Boolean policyItem = policyTmp.getReachabilityPolicy().getVerificationResult().isVerificationResult();
		return policyItem;
	}
