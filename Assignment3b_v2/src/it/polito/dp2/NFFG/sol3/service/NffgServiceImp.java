...
public class NffgServiceImp {
	...
	public void postSingleNffg(Nffg nffg) {
		...
		try {
			...
			Response labelRequest = target.path("resource/nodes/" + request.getId() + "/label")
	...
	public void postSinglePolicy(Policy policy) {
		Nffg nffg = nffgMap.get(policy.getReachabilityPolicy().getNffgName());
		if (nffg == null) {
			throw new ForbiddenException();
		}
		String srcNode = nodeMap.get(policy.getReachabilityPolicy().getSrcNode());
		if (srcNode == null) {
			throw new ForbiddenException();
		}
		String destNode = nodeMap.get(policy.getReachabilityPolicy().getDestNode());
		if (destNode == null) {
			throw new ForbiddenException();
		}
		policyMap.put(policy.getReachabilityPolicy().getPolicyName(), policy);
	}
